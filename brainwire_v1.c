...
static void enc_bit(Enc *e, uint16_t *p, int bit){
    ...
    if (!bit) { e->range = bound; *p += (4096-*p)>>5; }
    else { e->low += bound; e->range -= bound; *p -= *p>>5; }
...
static int dec_bit(Dec *d, uint16_t *p){
    ...
    if (d->code < bound) { d->range = bound; *p += (4096-*p)>>5; bit=0; }
    else { d->code -= bound; d->range -= bound; *p -= *p>>5; bit=1; }
...
static int do_encode(const char *in, const char *out){
    ...
    Enc e; enc_init(&e,fo);
    Model Mm, Me; model_init(&Mm); model_init(&Me);
...
static int do_decode(const char *in, const char *out){
    ...
    int32_t prev=0;
    for (long i=0;i<nsamp;i++){
        int32_t m=get_val(&d,&Mm), r=get_val(&d,&Me);
